...
class SitePagesController extends BaseController
{
    ...
    public function ViewIndex($request, $response, $args) {
        //
        return $this->container->php_view->render($response, 'admin/site/pages.phtml', [
    ...
    public function GetAll($request, $response, $args) {
        ...
        //
        $results = SitePages::GetAll($app_id);
        //var_dump($results); exit;
    ...
    public function GetRecord($request, $response, $args) {
        ...
        //
        $results = SitePages::GetRecordById( $app_id, $args['id'] );
        //var_dump($results); exit;
    ...
    public function UpsertRecord($request, $response, $args) {
        ...
        //
        $page_name = Helper::safeVar($request->getParsedBody(), 'page_name');
        $url = Helper::safeVar($request->getParsedBody(), 'url');
        $header_footer_id = Helper::safeVar($request->getParsedBody(), 'header_footer_id');
        $body = Helper::safeVar($request->getParsedBody(), 'body');
        ...
        //
        if ( !$page_name ){
            $results['error'] = "proporciona el nombre de la pagina";
            return $response->withJson($results, 200);
        }
        //
        if ( !$url ){
        ...
        //
        if ( !is_numeric($header_footer_id) ){
            $results['error'] = "proporciona el header/footer";
            return $response->withJson($results, 200);
        }
        ...
        $sp_res = Query::StoredProcedure([
            "stmt" => function(){
                return "{call usp_UpsertSitePage(?,?,?,?,?,?,?,?,?)}";
            },
            "exeptions_msgs" => [
                "default" => "Server Error, unable to do operation"
            ],
            "debug" => true,
            "params" => function() use($page_name, $url, $header_footer_id, $body, $active, $app_id, $record_id, &$param_new_id, &$param_mode){
                return [
                    //
                    array($page_name, SQLSRV_PARAM_IN),
                    array($url, SQLSRV_PARAM_IN),
                    array($header_footer_id, SQLSRV_PARAM_IN),
    ...
    public function DeleteRecord($request, $response, $args) {
        ...
        $results = Query::DoTask([
            "task" => "delete",
            "stmt" => "Delete FROM site_pages Where app_id = ? And id = ?;SELECT @@ROWCOUNT",
            "params" => [
                $app_id,
                $id
            ],
