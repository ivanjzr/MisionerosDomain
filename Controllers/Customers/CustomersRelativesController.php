...
//
class CustomersRelativesController extends BaseController
{
        ...
        $results = Query::PaginateRecordsV2(
            ...
            [
                ...
                "count_stmt" => function() use ($table_name, $search_clause){
                    //
                    return "Select COUNT(*) total From {$table_name} t Where account_id = ? and customer_parent_id = ? {$search_clause}";
                },
    ...
    public function PaginateRecordsPrimeReact($request, $response, $args) {
        ...
        $results = Query::ScrollPaginate(
            ...
            [
                "count_stmt" => function() use ($table_name, $search_clause){
                    //
                    return "Select COUNT(*) total From {$table_name} t Where t.customer_parent_id = ? {$search_clause}";
                },
                "records_stmt" => function($where_row_clause) use ($order_field, $order_direction, $table_name, $search_clause){
                    //
                    return "Select
    ...
    public function AddRecord($request, $response, $args) {
        ...
        //
        $name = Helper::getFirstTextOnly($body['name']);
        $relative_type_id = Helper::safeVar($body, 'relative_type_id');
        ...
        //
        if ( !is_numeric($relative_type_id) ){
        ...
        //
        $email_id = $customer_parent_id . "-" . generateRandomCode();
        //echo $email_id; exit;
        $relative_email = $email_id . "@temp.com";
        //echo $relative_email; exit;

        
        $person_res = Query::Single("Select * from v_customer_people Where app_id = ? and customer_parent_id = ? and name = ? and relative_type_id = ?", [$app_id, $customer_parent_id, $name, $relative_type_id]);
        //dd($person_res); 
        ...
        $insert_results = Query::DoTask([
            ...
            "stmt" => "
                  ...
                  ( account_id, app_id, customer_parent_id, name, email, relative_type_id, birth_date, edad_years, is_customer, datetime_created )
                  Values
                  ( ?, ?, ?, ?, ?, ?, ?, ?, 1, GETDATE() )
    ...
    public function UpdateRecord($request, $response, $args) {
        ...
        //
        $name = Helper::getFirstTextOnly($body['name_2']);
        $relative_type_id = Helper::safeVar($body, 'relative_type_id');
        $birth_date = Helper::safeVar($body, 'birth_date');
        $active = Helper::safeVar($body, 'active') ? 1 : 0;
        ...
        $person_res = Query::Single("Select * from v_customer_people Where app_id = ? and customer_parent_id = ? And name = ? And relative_type_id = ? And id != ?", [$app_id, $customer_parent_id, $name, $relative_type_id, $record_id]);
        //dd($person_res); 
        if ($person_res && isset($person_res['id'])){
            $results['error'] = "Ya existe un familiar con el mismo tipo y nombre"; 
            return $response->withJson($results, 200);
    ...
    public function DeleteRecord($request, $response, $args) {
        ...
        $results = Query::DoTask([
            "task" => "delete",
            "debug" => true,
            "stmt" => "Delete FROM contacts Where account_id = ? And app_id = ? And is_customer = 1 And customer_parent_id = ? And id = ?;SELECT @@ROWCOUNT",
            "params" => [
                $account_id,
                $app_id,
                $customer_parent_id,
